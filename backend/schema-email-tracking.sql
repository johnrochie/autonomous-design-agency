...
-- Index: Query emails by status and type
...
-- Email Logs RLS - Admins can read/write, clients can read their emails
...
CREATE POLICY "Clients can read their email logs"
  ON public.email_logs FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.projects
      INNER JOIN public.clients ON projects.client_id = clients.id
      WHERE clients.email = email_logs.to_email
    )
  );

CREATE POLICY "Only admins can insert email logs"
  ON public.email_logs FOR INSERT WITH CHECK (
    EXISTS (
      SELECT 1 FROM public.profiles
      WHERE profiles.id = auth.uid()
      AND profiles.role = 'admin'
...
DECLARE
  v_log_id UUID;
BEGIN
  INSERT INTO public.email_logs (to_email, subject, type, status, project_id, metadata)
  VALUES (p_to_email, p_subject, p_type, p_status, p_project_id, p_metadata)
...
-- ============================================
-- FUNCTION: Update email status (delivered, opened, etc.)
-- ============================================
