...
INSERT INTO public.cron_jobs (name, job_type, frequency, status, next_run, config) VALUES
  ...
  ('Email Sending', 'email_send', 'every_hour', 'idle', NOW(), '{"batch_size": 50, "retry_failed": true}'),
  ('Analytics Report', 'analytics_report', 'daily', 'idle', NOW() + INTERVAL '1 day', '{"include_platforms": ["twitter", "facebook", "linkedin"], "days": 7}'),
  ('Agent Heartbeat', 'agent_heartbeat', 'every_15_minutes', 'idle', NOW(), '{"timeout_minutes": 30}'),
...
CREATE OR REPLACE FUNCTION public.update_cron_job_status(p_job_id UUID, p_status TEXT, p_next_run TIMESTAMP WITH TIME ZONE DEFAULT NULL, p_duration_ms INTEGER DEFAULT NULL, p_error TEXT DEFAULT NULL) RETURNS VOID AS $$
BEGIN
  UPDATE public.cron_jobs SET status = p_status, last_run = NOW(), duration_ms = p_duration_ms, last_error = p_error, runs_count = runs_count + 1, success_count = CASE WHEN p_status = 'completed' THEN success_count + 1 ELSE success_count END, failure_count = CASE WHEN p_status = 'failed' THEN failure_count + 1 ELSE failure_count END, last_success = CASE WHEN p_status = 'completed' THEN NOW() ELSE last_success END, next_run = COALESCE(p_next_run, next_run), updated_at = NOW() WHERE id = p_job_id;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION public.log_cron_job(p_job_id UUID, p_status TEXT, p_output TEXT DEFAULT NULL, p_error TEXT DEFAULT NULL, p_metadata JSONB DEFAULT '{}') RETURNS UUID AS $$
DECLARE v_log_id UUID; v_duration_ms INTEGER;
BEGIN
  SELECT EXTRACT(EPOCH FROM (completed_at - started_at)) * 1000 INTO v_duration_ms FROM public.cron_job_logs WHERE cron_job_id = p_job_id AND id = (SELECT MAX(id) FROM public.cron_job_logs WHERE cron_job_id = p_job_id);
  INSERT INTO public.cron_job_logs (cron_job_id, status, output, error, metadata) VALUES (p_job_id, p_status, p_output, p_error, p_metadata) RETURNING id INTO v_log_id;
  RETURN v_log_id;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION public.calculate_next_run(p_frequency TEXT, p_last_run TIMESTAMP WITH TIME ZONE) RETURNS TIMESTAMP WITH TIME ZONE AS $$
DECLARE v_next_run TIMESTAMP WITH TIME ZONE;
BEGIN
  CASE p_frequency WHEN 'every_minute' THEN v_next_run := p_last_run + INTERVAL '1 minute'; WHEN 'every_5_minutes' THEN v_next_run := p_last_run + INTERVAL '5 minutes'; WHEN 'every_15_minutes' THEN v_next_run := p_last_run + INTERVAL '15 minutes'; WHEN 'every_hour' THEN v_next_run := p_last_run + INTERVAL '1 hour'; WHEN 'every_6_hours' THEN v_next_run := p_last_run + INTERVAL '6 hours'; WHEN 'every_12_hours' THEN v_next_run := p_last_run + INTERVAL '12 hours'; WHEN 'daily' THEN v_next_run := p_last_run + INTERVAL '1 day'; WHEN 'weekly' THEN v_next_run := p_last_run + INTERVAL '1 week'; WHEN 'monthly' THEN v_next_run := p_last_run + INTERVAL '1 month'; ELSE v_next_run := p_last_run + INTERVAL '1 hour'; END CASE;
  RETURN v_next_run;
END;
$$ LANGUAGE plpgsql;
